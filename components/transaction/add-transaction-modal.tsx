...
export function AddTransactionModal({ onClose }: { onClose?: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!amount || Number(amount) <= 0) { setError(t('enterValidAmount')); return; }
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => { setOpen(v); if (!v) reset(); }}>
      ...
      <DialogContent className="sm:max-w-md max-h-[90vh] overflow-y-auto border-border bg-card rounded-2xl p-0">
        <DialogHeader className="px-6 pt-6 pb-4 border-b border-border">
          <DialogTitle className="text-base font-semibold text-foreground">{t('newTransaction')}</DialogTitle>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-5">
          <div className="flex p-1 bg-secondary rounded-xl">
            {(['expense','income'] as const).map(v => (
              <button key={v} type="button" onClick={() => {
                ...
                className={`flex-1 py-2 text-sm font-semibold rounded-lg transition-all duration-200 ${type === v ? 'bg-card text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'}`}>
                {v === 'expense' ? `↓ ${t('expense')}` : `↑ ${t('income')}`}
              </button>
            ))}
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              <span className="absolute left-4 top-1/2 -translate-y-1/2 text-sm font-bold text-muted-foreground">{t('etb')}</span>
          ...
          <div className="space-y-2">
            <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">{t('category')}</label>
            <div className="grid grid-cols-3 gap-2 max-h-44 overflow-y-auto scrollbar-thin pr-1">
              {visible.map(cat => (
                <button key={cat.id} type="button" onClick={() => setCategory(cat.id)}
                  className={`flex flex-col items-center gap-1 p-2.5 rounded-xl border text-center transition-all duration-150 ${category === cat.id ? 'border-accent bg-accent/10 text-foreground' : 'border-border bg-secondary text-muted-foreground hover:border-border/80 hover:text-foreground'}`}>
                  <span className="text-xl">{cat.icon}</span>
                  <span className="text-[10px] font-medium leading-tight">{cat.label[lang]}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">{t('descOptional')}</label>
            <input placeholder={t('whatWasThisFor')} value={description} onChange={e => setDesc(e.target.value)}

...
import { Send, Sparkles, Wifi, User } from 'lucide-react';
...
export default function AssistantPage() {
  ...
  const [messages, setMessages] = useState<Message[]>([{
    id: '1', role: 'assistant',
    content: "Hello! I'm your AI financial assistant powered by Llama 3.3 on Groq. I have access to your real transaction data and can give personalised advice. What would you like to know?",
  }]);
  ...
  const send = async (text: string) => {
    ...
    try {
      ...
      const msg = err instanceof APIError ? err.message : 'Failed to get a response.';
      ...
      <div className="max-w-3xl mx-auto space-y-5 animate-fade-up">
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground ml-12">Powered by Groq — analysing your real financial data</p>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-border bg-card overflow-hidden flex flex-col" style={{ height: '62vh' }}>
          ...
          {messages.length === 1 && !loading && (
            <div className="px-5 py-3 border-t border-border bg-secondary/30">
              <p className="text-[11px] text-muted-foreground font-medium mb-2">Suggested questions:</p>
